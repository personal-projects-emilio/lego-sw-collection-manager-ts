...
const render = (ui: ReactElement, extendedRenderOptions?: ExtendedRenderOptions) => {
  window.history.pushState({}, 'Test page', extendedRenderOptions?.route || '/minifigs')

  const Wrapper: React.FC = ({ children }) => {
    return (
      <Provider store={initStore(extendedRenderOptions?.preloadedState || initialStoreMocked)}>
        <BrowserRouter>
          <LocalStorageMock items={extendedRenderOptions?.localStorageMocked || {}}>
            {children}
          </LocalStorageMock>
        </BrowserRouter>
      </Provider>
