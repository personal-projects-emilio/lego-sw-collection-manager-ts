...
describe('MinifigFormModal', () => {
  ...
  const handlers = [
    rest.put<MinifigsList>(`${process.env.REACT_APP_API_BASEURL}/*`, (req, res, ctx) => {
  ...
  test('manage minifig id errors', async () => {
    ...
    expect(
      await screen.findByText('This need to be a minifig reference (/^sw[0-9]{4}[abcds]?$/)')
    ).toBeInTheDocument()
    fireEvent.input(idInputElement, { target: { value: 'sw0003' } })
    expect(await screen.findByText('This minifig already exists')).toBeInTheDocument()
  })
  ...
  test('edit minifig name on server', async () => {
    ...
    render(<MinifigFormModal handleClose={handleClose} editMinifigData={mockedMinifigsList[0]} />, {
      preloadedState,
    })
  ...
  test('add minifig on server', async () => {
    ...
    fireEvent.input(
      screen.getByPlaceholderText('Minifig name (ex: Battle Droid Tan with Back Plate)'),
      { target: { value: 'Battle Droid Tan with Back Plate' } }
    )
    fireEvent.focus(screen.getByPlaceholderText('Character name (ex: Battle Droid)'))
