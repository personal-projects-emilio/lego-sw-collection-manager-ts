...
test('render radio button show filter', () => {
  ...
  const allShowInputElement = screen.getByDisplayValue('all') as HTMLInputElement
  const ownedShowInputElement = screen.getByDisplayValue('owned') as HTMLInputElement
...
test('render tag  filter autocomplete', async () => {
  render(<MinifigsFilters />, { preloadedState })
  const tagInputElement = screen.getByPlaceholderText('Filter by tag') as HTMLInputElement
